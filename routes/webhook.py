...
def after_insert_webhook():
    ...
    # Only perform subfolder creation and template uploads when the
    # main project folder was actually created by this request. If the
    # folder already existed, avoid re-running template uploads.
    created_folders = 0
    existing_folders = 0
    uploaded_files = 0
    skipped_files = 0
    ...
    if project_folder.get("_was_created"):
        ...
        created_folders = summary.get("created_count", 0)
        existing_folders = summary.get("existing_count", 0)

        # After folder creation, upload the pair of templates into
        # 05. Project -> 03. Project Scope using the project folder id.
        tpl_summary = drive_service.ensure_project_scope_templates(project_folder["id"]) or {}
        uploaded_files = tpl_summary.get("files_uploaded", 0)
        skipped_files = tpl_summary.get("files_skipped", 0)
        errors = tpl_summary.get("errors", 0)
        logger.info(
            "03. Project Scope templates: uploaded=%s skipped=%s",
            uploaded_files,
            skipped_files,
        )
    else:
        logger.info("Project folder already existed; skipped subfolders and template upload id=%s", project_folder.get("id"))
    ...
    if project_folder.get("_was_created") and errors:
        resp["errors"] = errors
        resp["message"] = "Folder processed (templates upload had errors)"
